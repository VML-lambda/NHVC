...
def phasemap_8bit(phasemap, inverted=True, model_name='NHVC', image_res = (1088, 1920)):
    ...
    if 'May' in model_name:
        slm_res = (1080, 1920)
        pad_left = pad_right = slm_res[1] // 2 - (image_res[1] - slm_res[1]) // 2
        pad_top = pad_bottom = slm_res[0] // 2 - (image_res[0] - slm_res[0]) // 2
        padding = ((pad_top, pad_bottom), (pad_left, pad_right))
        out_phase = np.pad(out_phase, padding, mode='reflect')
    if inverted:
...
def get_psnr_ssim_lpips(recon_amp, target_amp):
    """get PSNR and SSIM metrics"""
    psnrs, ssims, lpips = {}, {}, {}
    lpips_vgg = LPIPS(net='vgg')
    ssim = SSIM(gaussian_kernel=True, sigma=1.5, kernel_size=11, reduction='elementwise_mean', data_range=1.0, k1=0.01, k2=0.03)

    # amplitude
    psnrs['amp'] = psnr(target_amp, recon_amp)
    # ssims['amp'] = ssim(target_amp, recon_amp, channel_axis=2, data_range=1.0)

    target_amp_tensor = torch.from_numpy(target_amp)
    target_amp_tensor = target_amp_tensor.unsqueeze(0)
    target_amp_tensor = target_amp_tensor.permute(0, 3, 1, 2).float()

    recon_amp_tensor = torch.from_numpy(recon_amp)
    recon_amp_tensor = recon_amp_tensor.unsqueeze(0)
    recon_amp_tensor = recon_amp_tensor.permute(0, 3, 1, 2).float()

    ssims['amp'] = ssim(target_amp_tensor, recon_amp_tensor).item()
    lpips['amp'] = lpips_vgg(target_amp_tensor, recon_amp_tensor).item()
